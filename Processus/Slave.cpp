...
int main( int argc, char *argv[] )
{
    ...
    if (parent == MPI_COMM_NULL)
    {
        ...
        // ***** RUN ***** 
        // Scargo : 1 + width + 1 + height + 1 + width + 1 + height 
        // in ord : 1 + w + 1 + h + h + 1 + w + 1 l
        //float* neighbour = new float[4+2*(width+height)];
        MPI_Request request; 
        ...
        neighbourCells[0]       = new float[neighbourSizes[0]];
        neighbourCells[1]       = new float[neighbourSizes[1]];
        neighbourCells[2]       = new float[neighbourSizes[2]];
        neighbourCells[3]       = new float[neighbourSizes[3]];
        neighbourCells[4]       = new float[neighbourSizes[4]];
        neighbourCells[5]       = new float[neighbourSizes[5]];
        neighbourCells[6]       = new float[neighbourSizes[6]];
        neighbourCells[7]       = new float[neighbourSizes[7]];
        ...
        #pragma omp parallel shared(newCells) 
        {
            ...
            for (int i = 0; i <slaveWidth*slaveHeight; i++){            
                ...
                for (int j = 0; j < 9; j++){
                    ...
                    moyenne+=getCellVal(cells, neighbourCells, slaveWidth, slaveHeight, newX, newY); 
                }   

                newCells[i]=moyenne/9;
                //cout << newCells[i] << " - " << moyenne << " <br/> " << endl; 
            }
        }
        cells = newCells; 
